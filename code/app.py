...
def _pick_tx_hash_from_onchain(onchain_metadata: dict) -> str | None:
    """Match frontend FactCheck.jsx: registration dict or first successful registrations[].result."""
    if not isinstance(onchain_metadata, dict):
        return None
    reg = onchain_metadata.get("registration") or {}
    for key in ("tx_hash", "transaction_hash", "txHash", "hash"):
        v = reg.get(key)
        if isinstance(v, str) and v.strip():
            return v.strip()
    for entry in onchain_metadata.get("registrations") or []:
        ...
        for key in ("tx_hash", "transaction_hash", "txHash", "hash"):
            ...
            if isinstance(v, str) and v.strip():
                return v.strip()
    for key in ("tx_hash", "transaction_hash", "txHash", "hash"):
        v = onchain_metadata.get(key)
        if isinstance(v, str) and v.strip():
            return v.strip()
    return None


def _pick_block_number_from_onchain(onchain_metadata: dict):
    if not isinstance(onchain_metadata, dict):
        return None
    reg = onchain_metadata.get("registration") or {}
    bn = reg.get("block_number")
    if bn is not None:
        return bn
    for entry in onchain_metadata.get("registrations") or []:
        if not isinstance(entry, dict):
            continue
        res = entry.get("result") or {}
        if isinstance(res, dict) and res.get("block_number") is not None:
            return res.get("block_number")
    return None


def _build_blockchain_card_html(claim_text: str, onchain_metadata: dict = None):
    """claim_text is kept for call-site compatibility; on-chain hash comes from onchain_metadata only."""
    _ = (claim_text or "").strip()
    onchain_metadata = onchain_metadata or {}
    tx_hash = _pick_tx_hash_from_onchain(onchain_metadata)
    block_number = _pick_block_number_from_onchain(onchain_metadata)
    if not tx_hash:
        tx_hash = "কোনো অন-চেইন লেনদেন রেকর্ড হয়নি (রেজিস্ট্রি ব্রিজ অপ্রাপ্য বা নিবন্ধন ব্যর্থ হতে পারে)"
