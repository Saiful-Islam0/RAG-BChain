...
class ImageFactChecker:
    ...
    def __init__(self, upload_dir: str = "uploaded_images", metadata_dir: str = "image_metadata"):
        ...
        if EASYOCR_AVAILABLE:
            try:
                # Fix macOS SSL cert issue so EasyOCR can download models
                import ssl
                import certifi
    ...
    def _normalize_image_orientation(self, image_path: str) -> str:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                # Only re-save if rotation was actually applied
                if needs_rotation:
                    normalized_path = image_path.replace('.', '_normalized.')
                    img.save(normalized_path, quality=98, optimize=True)
    ...
    def _save_image_metadata(self, metadata: Dict, image_id: str):
        ...
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(metadata, f, indent=2, ensure_ascii=False, default=self._json_default)
    ...
    def _preprocess_for_ocr(self, image):
        ...
        # Upscale small images so text is at least ~40px tall
        if max(h, w) < 1000:
            scale = 1500 / max(h, w)
        ...
        # Adaptive threshold to handle mixed backgrounds (white-on-red, black-on-white)
    ...
    def _extract_with_easyocr(self, image_path: str) -> Dict:
        ...
        try:
            ...
            # Run OCR on both raw and preprocessed, pick best
            results_raw = self.ocr_reader.readtext(image)
            results_pre = self.ocr_reader.readtext(preprocessed)
            
            # Pick whichever yields more high-confidence text
            def _score(results):
                return sum(c for (_, _, c) in results if c > 0.3)
            results = results_pre if _score(results_pre) >= _score(results_raw) else results_raw
    ...
    def _extract_with_tesseract(self, image_path: str) -> Dict:
        ...
        try:
            ...
            preprocessed = self._preprocess_for_ocr(image)
            # Tesseract works best with the preprocessed binary image
            ...
            # Also try raw image and pick the better result
            image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            text_raw = pytesseract.image_to_string(image_rgb, config=custom_config)
            if len(text_raw.strip()) > len(text.strip()):
                text = text_raw
                image_for_ocr = image_rgb
            
            # Get detailed data for confidence
            data = pytesseract.image_to_data(image_for_ocr, config=custom_config, output_type=pytesseract.Output.DICT)
    ...
    def _extract_key_news_text(self, ocr_result: Dict, image_width: Optional[int] = None, image_height: Optional[int] = None) -> Dict:
        """
        Heuristic filter to remove ad-like / watermark text from photo-cards before RAG.
        Returns: { key_text, kept_count, dropped_count, drop_reasons, raw_len, key_len }
        """
        raw_text = (ocr_result or {}).get("ocr_text") or ""
        segments = (ocr_result or {}).get("segments") or []

        def has_bn(s: str) -> bool:
            return any('\u0980' <= ch <= '\u09FF' for ch in (s or ""))

        def has_urlish(s: str) -> bool:
            s2 = (s or "").lower()
            return ("http://" in s2) or ("https://" in s2) or ("www." in s2) or (".com" in s2) or (".net" in s2) or (".bd" in s2)

        def has_phoneish(s: str) -> bool:
            digits = sum(ch.isdigit() for ch in (s or ""))
            return digits >= 7

        def digit_heavy_noise(s: str) -> bool:
            s2 = (s or "").strip()
            if not s2:
                return False
            digits = sum(ch.isdigit() for ch in s2)
            letters = sum(ch.isalpha() for ch in s2)
            # Common for ads: "100%", "50% OFF", model numbers, etc.
            has_pct = "%" in s2 or "％" in s2
            # If text contains % and is short, it is almost always ad/noise
            # (even if OCR adds a few stray letters)
            if has_pct and digits >= 1 and len(s2) <= 28:
                return True
            # If text is mostly digits/symbols, drop it
            if has_pct and letters <= 2:
                return True
            if digits >= 3 and letters <= 2 and len(s2) <= 12:
                return True
            if digits > 0 and (digits / max(len(s2), 1)) >= 0.45 and letters <= 3:
                return True
            return False

        ad_keywords = {
            "offer", "discount", "sale", "free", "subscribe", "follow", "like", "share",
            "call", "hotline", "coupon", "shop", "order",
            "refrigerator", "fridge", "mobile", "camera", "laptop",
        }
        watermark_keywords = {
            "jamuna",
            "vision",
        ...
        kept = []
        dropped = []
        drop_reasons = {"low_conf": 0, "url": 0, "phone": 0, "ad_kw": 0, "watermark": 0, "tiny_corner": 0, "other": 0}
        kept_sample = []
        dropped_sample = []

        # If OCR didn't provide segments, just return raw text
        if not segments:
            return {
                "key_text": raw_text.strip(),
                "kept_count": 0,
                "dropped_count": 0,
                "drop_reasons": drop_reasons,
                "raw_len": len(raw_text),
                "key_len": len(raw_text.strip()),
            }
        ...
        for seg in segments:
            try:
                ...
                if conf < 0.35:
                    ...
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "low_conf"})
                    continue
                lower = text.lower()
                if has_urlish(text):
                    drop_reasons["url"] += 1
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "url"})
                    continue
                if has_phoneish(text):
                    drop_reasons["phone"] += 1
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "phone"})
                    continue
                if digit_heavy_noise(text):
                    # Treat as ad/label noise bucket
                    drop_reasons["ad_kw"] += 1
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "digit_noise"})
                    continue
                if any(k in lower for k in ad_keywords):
                    drop_reasons["ad_kw"] += 1
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "ad_kw"})
                    continue
                if any(k in lower for k in watermark_keywords):
                    drop_reasons["watermark"] += 1
                    dropped.append(text)
                    if len(dropped_sample) < 8:
                        dropped_sample.append({"t": text[:80], "r": "watermark"})
                    continue

                # If we have image dims + bbox, drop tiny corner text (common for logos/ads)
                if image_width and image_height and bbox and isinstance(bbox, (list, tuple)) and len(bbox) >= 4:
                    ...
                    if xs and ys:
                        ...
                        box_w = max(1.0, float(maxx - minx))
                        box_h = max(1.0, float(maxy - miny))
                        area = box_w * box_h
                        img_area = float(image_width * image_height)
                        near_edge = (minx < image_width * 0.08) or (maxx > image_width * 0.92) or (miny < image_height * 0.08) or (maxy > image_height * 0.92)
                        if near_edge and (area / max(img_area, 1.0)) < 0.0025:
                            drop_reasons["tiny_corner"] += 1
                            dropped.append(text)
                            if len(dropped_sample) < 8:
                                dropped_sample.append({"t": text[:80], "r": "tiny_corner"})
                            continue

                kept.append(text)
                if len(kept_sample) < 8:
                    kept_sample.append(text[:80])
            except Exception:
                drop_reasons["other"] += 1
                continue

        # Prefer Bengali segments when present (photo-cards are often Bengali with some English ads)
        kept_bn = [t for t in kept if has_bn(t)]
        candidate = kept_bn if kept_bn else kept
    ...
    def verify_image_news(self, image_input) -> Tuple[str, str, str, str, str, str, int, str, List[Dict], Dict]:
        """
        Enhanced function to verify news from image with visual analysis
        Returns:
          (claim, verdict_english, verdict_original, ocr_text, caption, visual_summary,
           credibility_score, classification, evidence_sources, onchain_metadata)
        """
        try:
            ...
            key_info = self._extract_key_news_text(
                ocr_result,
                image_width=(metadata.get("dimensions") or {}).get("width"),
                image_height=(metadata.get("dimensions") or {}).get("height"),
            )
            key_text = (key_info.get("key_text") or "").strip()
            ...
            metadata['evidence_sources_count'] = len(evidence_sources or [])
            metadata['ocr_key_text'] = key_text[:1200] if key_text else ""
